...
`Components/Market/TradeInsightsWidget.jsx` renders above `WatchlistPanel` in the trader workspace branch (`workspaceMode || !isSuperAdmin`, so it also shows on the superadmin's `/admin/workspace`). It fetches `GET /market-backtest/report/insights` on mount and, if the account is eligible (≥30 closed trades) and at least one coaching heuristic fired, shows only the single highest-severity tip as a one-line, dismissible teaser linking to `/trade-report` — the workspace is already dense with the chart and watchlist, so it deliberately doesn't show the full up-to-3-tip list `TradeReport.jsx` shows (see [Trade reports and journals](trade-reports-and-journals.md)). It renders nothing while ineligible, tip-free, or dismissed; dismissal is local component state, not persisted, so the tip reappears on the next visit if it still fires.

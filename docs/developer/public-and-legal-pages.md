...
- The nav's brand block (`Link` wrapping the logo and "BacktradeLab" text) and the login-menu wrapper on its right both used `flex-1`, splitting the row's width evenly between them — on mobile widths this left the brand name only its half-share of the row (minus the logo icon and gap) to work with, so the `truncate` class it already carries actually kicked in and clipped "BacktradeLab" to "BacktradeL…". The login-menu wrapper doesn't need flexible width at all (it's an intrinsically-sized button), so it's `shrink-0` now — the brand block keeps the row's full remaining space and no longer truncates on any supported viewport. If a nav item is ever added between them, keep exactly one side `flex-1`/`min-w-0` (for truncation to be reachable) and give any purely intrinsic-width sibling `shrink-0` instead of a second `flex-1`.

...
- Verify embedded and fullscreen Indicators menus in dark and white themes show only name toggles for Volume, SMA, EMA, and RSI plus the enabled-indicator count; no period, color, line-width, or pane-size controls should appear in the add-indicator menu.
- Enable Volume, SMA, EMA, and RSI and confirm only SMA/EMA have compact clickable targets in the main pane and RSI follows its pane after resizing; Volume must not show a left-side settings target or contextual panel. Click each SMA/EMA/RSI target and rendered series and verify only that indicator's contextual panel opens beside/below the selection, flips above when it would overflow, closes when clicking elsewhere, and stays within chart bounds. Confirm period, line color, line width, RSI pane size, close, and remove actions work in embedded/fullscreen dark and light modes and persist only for the authenticated user.

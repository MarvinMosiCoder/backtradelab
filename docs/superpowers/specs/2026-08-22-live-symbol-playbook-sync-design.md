...
> **Implementation note (added after planning):** the mechanism below describes two raw, purpose-built `BroadcastChannel`s with structured payloads and manual state-merging on each end. During implementation planning, a simpler option was found: this codebase already has an established `window` `CustomEvent` convention for cross-component notifications (`backtradelab-alert-triggered`, `backtradelab-active-symbol-change`, etc.), including a `backtradelab-symbols-changed` event that was already being dispatched but had no listeners — a gap independently documented in `docs/developer/trading-chart.md`. The implementation reuses that convention via a small shared helper (`resources/js/utils/crossTabSync.js`, `broadcastChange`/`subscribeToChange`) that also relays across tabs via `BroadcastChannel`, rather than introducing a second, parallel channel-based mechanism. The **outcome** described below (live updates, no reload, cross-tab) is unchanged; only the wire mechanism differs — no structured payloads are passed, listeners just re-fetch via the existing fetch functions. See `docs/superpowers/plans/` history or the implementation plan for the revised design.

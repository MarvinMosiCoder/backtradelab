...
import { AlertTriangle, Bell, CreditCard, LayoutDashboard, LogOut, Menu, MessageSquareText, Moon, Settings, ShieldCheck, Sun, Users, X } from 'lucide-react';
...
export default function AdminNavbar() {
    ...
    useEffect(() => { getAppLogo().then(setLogo); }, []);
    const toggleTheme = () => {
    ...
    return <header className={`flex h-14 items-center border-b px-3 sm:px-4 ${isDark ? 'border-[#2a2e39] bg-[#131722] text-[#d1d4dc]' : 'border-slate-200 bg-white text-slate-800'}`}>
        <button type="button" onClick={() => toggleSidebar()} className="mr-2 rounded-md p-2 hover:bg-white/10" aria-label="Toggle admin navigation"><Menu size={18}/></button>
        <Link href="/dashboard" className="flex shrink-0 items-center gap-2 pr-4"><span className="flex h-8 w-8 items-center justify-center overflow-hidden rounded-md bg-[#2962ff]">{logo?<img src={logo} alt="BacktradeLab" className="h-full w-full object-contain p-1"/>:<ShieldCheck size={17} className="text-white"/>}</span><span className="hidden sm:block"><span className="block text-sm font-bold leading-none">BacktradeLab</span><span className="mt-1 block text-[9px] font-semibold uppercase tracking-[.2em] text-[#787b86]">Admin console</span></span></Link>
        <nav className="hidden items-center gap-1 lg:flex"><Link href="/dashboard" className={navLink}><LayoutDashboard size={14}/>Overview</Link><Link href="/users" className={navLink}><Users size={14}/>Users</Link><Link href="/admin/feedback" className={navLink}><MessageSquareText size={14}/>Feedback</Link><Link href="/admin/subscriptions" className={navLink}><CreditCard size={14}/>Payments</Link><Link href="/admin/subscription-plans" className={navLink}><CreditCard size={14}/>Pricing</Link><Link href="/admin/payment-settings" className={navLink}><Settings size={14}/>Payment setup</Link><Link href="/settings" className={navLink}><Settings size={14}/>Settings</Link></nav>
        <div className={`ml-auto flex items-center gap-1 border-l pl-2 ${isDark?'border-[#2a2e39]':'border-slate-200'}`}><Link href="/notifications/view-all-notifications" className="relative rounded-md p-2 text-[#787b86] hover:bg-white/10" aria-label="Notifications"><Bell size={16}/>{Number(auth?.unread_notifications)>0&&<span className="absolute right-1 top-1 h-1.5 w-1.5 rounded-full bg-red-500"/>}</Link><button type="button" onClick={toggleTheme} className="rounded-md p-2 text-[#787b86] hover:bg-white/10" aria-label="Toggle theme">{isDark?<Sun size={16}/>:<Moon size={16}/>}</button><Link href="/profile" className="hidden rounded-md px-2 py-1.5 text-right hover:bg-white/5 sm:block"><span className="block max-w-32 truncate text-xs font-semibold">{auth?.user?.name}</span><span className="block text-[9px] uppercase tracking-wider text-[#787b86]">Superadmin</span></Link><button type="button" onClick={()=>setShowLogoutModal(true)} className="rounded-md p-2 text-[#787b86] hover:bg-red-500/10 hover:text-red-400" aria-label="Sign out"><LogOut size={16}/></button></div>

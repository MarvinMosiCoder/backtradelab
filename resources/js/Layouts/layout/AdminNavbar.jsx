...
import { AlertTriangle, Bell, LayoutDashboard, LogOut, Menu, Moon, ShieldCheck, Sun, Users, X } from 'lucide-react';
...
export default function AdminNavbar() {
    ...
    return <header className={`flex h-14 items-center border-b px-3 sm:px-4 ${isDark ? 'border-[#2a2e39] bg-[#131722] text-[#d1d4dc]' : 'border-slate-200 bg-white text-slate-800'}`}>
        ...
        <Link href="/dashboard" className="flex shrink-0 items-center gap-2 pr-4"><span className="flex h-8 w-8 items-center justify-center overflow-hidden rounded-md bg-[#2962ff]">{logo?<img src={logo} alt={appName} className="h-full w-full object-contain p-1"/>:<ShieldCheck size={17} className="text-white"/>}</span><span className="hidden sm:block"><span className="block text-sm font-bold leading-none">{appName}</span><span className="mt-1 block text-[9px] font-semibold uppercase tracking-[.2em] text-[#787b86]">Admin console</span></span></Link>

...
const AppNavbar = () => {
    ...
    const readNotification = async (e, id) => {
        try {
            ...
            setNotifications((prevNotifications) => prevNotifications.map((notification) => (notification.id === id ? { ...notification, is_read: true } : notification)));
    ...
    const handleNavbarSymbolChange = (event) => {
        ...
        try {
            ...
            window.dispatchEvent(
                new CustomEvent("backtradelab-active-symbol-change", {
                    detail: symbol,
                }),
            );
    ...
    return (
        <>
            <div className="flex flex-col lg:flex-row">
                <div className={`${theme} md:hidden lg:hidden w-full h-[50px] lg:h-[60px] border-b-[1px] lg:w-[299px] ${!["bg-skin-black"].includes(theme) ? "border-gray-200" : "border-gray-700"}`} ref={menuRef}>
                    <Link href={`${window.location.origin}/dashboard`}>
                        <div className="flex gap-x-4 items-center justify-center lg:justify-start px-[20px] py-[10px] lg:py-[15px]">
                            {applogo ? (
                                <img src={applogo} className="w-7 h-7 cursor-pointer duration-500" alt="App Logo" />
                            ) : (
                                <div className="flex justify-center items-center h-full">
                                    <div className="h-7 w-7 rounded-3xl bg-gray-100 animate-pulse" />
                                </div>
                            )}
                            <div className={`${theme === "bg-skin-black" ? "text-gray-300" : theme === "bg-skin-white" ? textColor : textColorActive} font-medium`}>
                ...
                <div className={`${theme === "bg-skin-black" ? "bg-[#131722] text-[#d1d4dc]" : theme} w-full h-[60px] border-b ${!["bg-skin-black"].includes(theme) ? "border-gray-200" : "border-[#2a2e39]"} flex items-center justify-end px-5 py-7 select-none shadow-lg`} ref={menuRef}>
                    <Link href={`${window.location.origin}/dashboard`} className="mr-auto w-auto hidden md:block lg:block">
                        <div className="flex gap-x-4 items-center justify-center lg:justify-start py-[10px] lg:py-[15px]">
                            {applogo ? (
                                <img src={applogo} className="w-7 h-7 cursor-pointer duration-500" alt="App Logo" />
                            ) : (
                                <div className="flex justify-center items-center h-full">
                                    <div className="h-7 w-7 rounded-3xl bg-gray-200 animate-pulse" />
                                </div>
                            )}
                            <div className={`${theme === "bg-skin-black" ? "text-gray-300" : theme === "bg-skin-white" ? textColor : textColorActive} font-medium`}>
                                <p className="font-semibold text-[15px]">{appname}</p>
                    ...
                    {!isSuperAdmin && (
                        <div className={`mr-3 hidden items-center gap-2 rounded-md border px-2 py-1.5 lg:flex ${theme === "bg-skin-black" ? "border-gray-700 bg-black-table-color text-gray-300" : "border-gray-200 bg-white text-gray-700"}`}>
                            <Link href="/dashboard" className={`rounded px-2 py-1 text-xs font-semibold ${theme === "bg-skin-black" ? "hover:bg-skin-black-light" : "hover:bg-gray-100"}`}>
                                Chart
                            </Link>
                            <Link href="/trade-report" className={`rounded px-2 py-1 text-xs font-semibold ${theme === "bg-skin-black" ? "hover:bg-skin-black-light" : "hover:bg-gray-100"}`}>
                                PnL
                            </Link>
                            <select className={`h-8 max-w-[190px] rounded border px-2 text-xs outline-none ${theme === "bg-skin-black" ? "border-gray-700 bg-skin-black text-gray-200" : "border-gray-200 bg-gray-50 text-gray-700"}`} value={getSymbolKey(activeNavbarSymbol)} onChange={handleNavbarSymbolChange} title="Saved symbols">
                    ...
                    <div className="flex items-center gap-4 pl-[16px] py-10">
                        <div
                            ...
                            ${["bg-skin-white", "bg-skin-blue", "bg-skin-blue-light"].includes(theme) ? "text-black" : "text-white"} 
                        ...
                        <div className="relative cursor-pointer" onClick={handleToggleNotification}>
                            <div className={`flex items-center justify-center ${theme == "bg-skin-white" ? bgColor : theme} p-3 rounded-full border-[2px] ${borderColor} w-10 h-10`}>
                                <i className={`fa fa-bell ${textColorActive} text-[19px]`}></i>
                                {unreadnNotifications > 0 && <div className={`absolute top-[15px] right-[16px] bg-red-500 ${textColorActive} text-[10px] font-bold rounded-full w-3 h-3 flex items-center justify-center transform translate-x-1/2 -translate-y-1/2`}>{unreadnNotifications}</div>}
                            </div>
                        </div>
                        ...
                        {showNotifications && (
                            <div className={`absolute z-90 right-1 top-[113px] md:lg:top-[65px] lg:top-[65px] ${theme === "bg-skin-black" ? "bg-skin-black" : "bg-white"}  py-3 rounded-[5px] pop-up-boxshadow z-[100] w-[332px] max-h-[390px]`}>
                                <div className="flex items-center gap-5 border-b-[1px] px-3 pb-2 min-w-75 w-[332px]">
                                    <span className={`${!["bg-skin-black"].includes(theme) ? textColor : textColorActive}`}>
                                        <i className="fa fa-info-circle font-semibold"></i> Notifications
                                    </span>
                                </div>
                                <div className="mb-2">
                                    <div className="max-h-[250px] overflow-y-auto overflow-x-hidden">
                                        {" "}
                                        {/* Restrict height and enable scrolling */}
                                        {notifications.map((notification) => (
                                            <Notification key={notification.id} id={notification.id} message={notification.content} type={notification.type} isRead={notification.is_read} onClick={readNotification} theme={theme} created={notification.created_at} themeColor={theme} textColorActive={textColorActive} textColorMain={textColor} />
                                        ))}
                                    </div>
                                    <div className="pl-2 mt-5 pr-2">
                                        <Button href="/notifications/view-all-notifications" type="link" extendClass={(theme === "bg-skin-white" ? `bg-skin-white-hover` : theme) + ` px-[85px] w-full border-[1px] ${borderColor}`} fontColor="text-white">
                        ...
                        <div
                            className={`flex items-center gap-4 border-l-[1px] pl-5 py-[8px] ${theme === "bg-skin-black" ? "border-gray-700" : "border-gray-200"} cursor-pointer`}
                    ...
                    {showMenu && (
                        <div className={`absolute z-90 right-1 top-[113px] md:lg:top-[65px] lg:top-[65px] ${theme === "bg-skin-black" ? "bg-skin-black" : "bg-white"} py-3 rounded-[5px] pop-up-boxshadow z-[100] w-[332px]`}>
                            ...
                            >
                                ...
                                <span className="font-poppins text-[16px] ml-2">Profile</span>
                            </Link>
                            <Link
                            ...
                            >
                                ...
                                <span className="font-poppins ml-2">Change password</span>
                            </Link>
                            <div
                                className={`border-t-[1px] px-5 py-2 flex items-center cursor-pointer 
                            ...
                            >
                                ...
                                <span className="font-poppins text-[16px] ml-2">Dark mode</span>
                            </div>
            ...
            <Modal show={showModalTheme} onClose={handleCloseModal} title="Theme" width="xl" fontColor={textColorActive} icon="fa fa-moon" btnIcon="fa fa-refresh" withButton="true" onClick={handleThemeUpdate}>
                <form>
                    <div className="p-2">
                        <div className="space-y-2">
                            <label className="flex items-center justify-between space-x-2">
                                <span className={`${textColor}`}>Dark mode</span>
                                <input type="radio" name="theme" value="skin-black" onChange={handleRadioInput} className="form-radio h-5 w-5 text-blue-600 ml-auto" />
                            </label>
                            <label className="flex items-center justify-between space-x-2">
                                <span className={`${textColor}`}>Light mode</span>
                                <input type="radio" name="theme" value="skin-white" onChange={handleRadioInput} className="form-radio h-5 w-5 text-blue-600 ml-auto" />
                            </label>
                            <label className="flex items-center justify-between space-x-2">
                                <span className={`${textColor}`}>System default</span>
                                <input type="radio" name="theme" value={auth.sessions.theme_color} onChange={handleRadioInput} className="form-radio h-5 w-5 text-blue-600 ml-auto" />
                            </label>
                        </div>
                    </div>
                </form>

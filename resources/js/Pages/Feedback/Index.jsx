...
export default function FeedbackIndex() {
    ...
    return <><Head title="Customer Support" /><div className={`mx-auto max-w-6xl space-y-4 py-2 ${isDark ? 'text-[#d1d4dc]' : 'text-slate-900'}`}>
        <div><div className="text-xs font-bold uppercase tracking-[.18em] text-[#2962ff]">Customer care</div><h1 className={`mt-1 text-2xl font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>Customer support</h1><p className="mt-1 text-sm text-[#787b86]">Get help with payments, subscriptions, account access, or product issues.</p></div>
        ...
        <div className="grid gap-4 lg:grid-cols-[1.05fr_.95fr]">
            <div className={`rounded-xl border p-5 ${panel}`}>
                <div className="flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-wider text-[#787b86]">
                    <span className={step === 1 ? 'text-[#2962ff]' : ''}>1. Category</span>
                    <ChevronRight size={12}/>
                    <span className={step === 2 ? 'text-[#2962ff]' : ''}>2. Details</span>
                </div>
                ...
                {step === 1 ? <>
                    ...
                    <button type="button" onClick={backToCategories} className="flex items-center gap-1.5 text-xs font-semibold text-[#5b8cff]"><ArrowLeft size={14}/>Change category</button>
                    <div className="mt-3 flex items-center gap-2">{CategoryIcon && <CategoryIcon size={16} className="text-[#5b8cff]"/>}<h2 className={`text-sm font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>{selectedCategory?.[1]}</h2></div>
                    ...
                    {form.category === 'payment' && <div className="mt-4 grid gap-3 sm:grid-cols-2">
                        <label className="text-xs font-semibold">Which payment is this about?
                            <select value={form.subscription_request_id} onChange={(e) => setForm((c) => ({ ...c, subscription_request_id: e.target.value }))} className={`mt-1.5 h-11 w-full rounded-lg border px-3 text-sm outline-none focus:border-[#2962ff] ${field}`}>
                        ...
                        <label className="text-xs font-semibold">What happened?
                            <select value={form.payment_reason_code} onChange={(e) => setForm((c) => ({ ...c, payment_reason_code: e.target.value }))} className={`mt-1.5 h-11 w-full rounded-lg border px-3 text-sm outline-none focus:border-[#2962ff] ${field}`}>

...
function PaymentStepper({ status, polling, dark }) {
  ...
  const circleClass = state => {
    ...
    if (state === 'done') return 'bg-[#2962ff] text-white';
    if (state === 'active') return 'border-2 border-[#2962ff] text-[#2962ff] animate-pulse';
  ...
  return <div className="flex items-center">
    {steps.map((step, index) => { const state = stateFor(index); return <React.Fragment key={step.label}>
      ...
      {index < steps.length - 1 && <div className={`mx-2 h-0.5 w-6 sm:w-12 ${state === 'done' || state === 'success' ? 'bg-[#2962ff]' : dark ? 'bg-[#2a2e39]' : 'bg-slate-200'}`}/>}
    </React.Fragment>; })}
  </div>;
}

...
import { CalendarClock, Clock3, CreditCard, FileImage, MessageCircle, RefreshCw, ShieldCheck } from 'lucide-react';
...
const tones = {
  active: 'border-emerald-500/30 bg-emerald-500/10 text-emerald-500', paid: 'border-emerald-500/30 bg-emerald-500/10 text-emerald-500',
  available: 'border-blue-500/30 bg-blue-500/10 text-blue-500', trial: 'border-blue-500/30 bg-blue-500/10 text-blue-500',
  creating: 'border-amber-500/30 bg-amber-500/10 text-amber-500', pending: 'border-amber-500/30 bg-amber-500/10 text-amber-500',
  failed: 'border-red-500/30 bg-red-500/10 text-red-500', rejected: 'border-red-500/30 bg-red-500/10 text-red-500',
  expired: 'border-slate-500/30 bg-slate-500/10 text-slate-500', archived: 'border-slate-500/30 bg-slate-500/10 text-slate-500',
...
export default function UserIndex({ subscription }) {
  ...
  const paymentResult = new URL(url, window.location.origin).searchParams.get('payment');
  ...
  return <><Head title="Subscription"/>
    ...
    <div className={`mx-auto max-w-6xl space-y-5 ${dark ? 'text-white' : 'text-slate-900'}`}>
      {(notice || paymentResult) && <div role="status" className={`rounded-xl border p-4 text-sm ${tones[paymentResult === 'paid' ? 'paid' : paymentResult === 'failed' ? 'failed' : 'pending']}`}>{notice || (paymentResult === 'paid' ? 'Payment verified. Your replay access is active.' : paymentResult === 'cancelled' ? 'Checkout was cancelled. No access was granted.' : 'Your payment is being verified. You can safely check its status below.')}</div>}
      <section className={`overflow-hidden rounded-2xl border ${card}`}>
        <div className={`p-5 sm:p-7 ${dark ? 'bg-gradient-to-r from-[#172554] via-[#131722] to-[#0b0e14]' : 'bg-gradient-to-r from-blue-50 via-white to-slate-50'}`}><div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-center"><div><div className={`inline-flex items-center gap-2 rounded-full border px-3 py-1 text-xs font-bold ${tones[subscription.status]}`}><span className="h-1.5 w-1.5 rounded-full bg-current"/>{statusLabel}</div><h1 className="mt-4 text-2xl font-bold">Your replay subscription</h1><p className={`mt-2 max-w-xl text-sm ${secondary}`}>Manage replay access and review one-time PayMongo transactions.</p></div><button onClick={() => setShowPlans(true)} className="h-11 rounded-lg bg-[#2962ff] px-5 text-sm font-bold text-white hover:bg-blue-600">{subscription.trialAvailable ? 'Activate free trial' : subscription.allowed ? 'View plans' : 'Renew replay access'}</button></div></div>
      ...
      <section><div className="mb-3"><h2 className="text-lg font-bold">Payment history</h2><p className={`text-sm ${muted}`}>PayMongo transactions and preserved read-only manual records.</p></div><div className={`overflow-hidden rounded-xl border ${card}`}>
        {subscription.requests.length ? <div className={`divide-y ${dark ? 'divide-[#2a2e39]' : 'divide-slate-200'}`}>{subscription.requests.map(item => <article key={item.id} className="grid gap-4 p-4 lg:grid-cols-[1fr_auto] lg:items-center sm:p-5"><div><div className="flex flex-wrap items-center gap-2"><span className="font-bold capitalize">{item.plan} plan</span><span className={`rounded-full border px-2 py-0.5 text-[10px] font-bold uppercase ${tones[item.status] || tones.pending}`}>{item.status}</span><span className={`rounded-full border px-2 py-0.5 text-[10px] font-bold uppercase ${border} ${muted}`}>{item.legacy ? 'manual · archived' : `${item.provider} · ${item.mode}`}</span></div><div className={`mt-2 grid gap-1 text-xs sm:grid-cols-2 xl:grid-cols-3 ${muted}`}><span>Amount: <b className={secondary}>{money(item)}</b></span><span>Reference: <b className={secondary}>{item.payment_reference || item.provider_checkout_id || '—'}</b></span><span>Submitted: {formatDate(item.created_at)}</span><span>Paid: {formatDate(item.paid_at)}</span><span>Duration: {item.duration_days ? `${item.duration_days} days` : 'Legacy plan record'}</span><span>Method: <span className="capitalize">{(item.payment_method || item.provider || 'manual').replaceAll('_', ' ')}</span></span></div>{(item.provider_status_message || item.admin_notes) && <div className={`mt-3 rounded-lg border p-3 text-xs ${border} ${secondary}`}>{item.provider_status_message || item.admin_notes}</div>}</div><div className="flex flex-wrap gap-2">{item.payment_proof_url && <a href={item.payment_proof_url} target="_blank" rel="noreferrer" className={`flex items-center gap-2 rounded-lg border px-3 py-2 text-xs font-semibold ${border}`}><FileImage size={15}/>View proof</a>}{item.legacy && item.messages_count > 0 && <button onClick={() => setChatRequest(item)} className={`flex items-center gap-2 rounded-lg border px-3 py-2 text-xs font-semibold ${border}`}><MessageCircle size={15}/>Archived chat ({item.messages_count})</button>}{item.provider === 'paymongo' && item.status === 'pending' && <button disabled={checking === item.id} onClick={() => checkStatus(item)} className="flex items-center gap-2 rounded-lg bg-[#2962ff] px-3 py-2 text-xs font-semibold text-white disabled:opacity-50"><RefreshCw size={15} className={checking === item.id ? 'animate-spin' : ''}/>Check status</button>}</div></article>)}</div> : <div className="p-10 text-center"><CreditCard className={`mx-auto ${dark ? 'text-[#434955]' : 'text-slate-300'}`}/><h3 className="mt-3 font-semibold">No transactions yet</h3><p className={`mt-1 text-sm ${muted}`}>Completed and pending checkouts will appear here.</p></div>}
      </div></section>
    </div>
  </>;
}

        ...
        >
            <article
                className={`group relative overflow-hidden rounded-xl border p-5 transition-all duration-300 hover:-translate-y-1 ${
                    isDark
                        ? 'border-[#2a2e39] bg-[#131722] hover:border-[#2962ff]/60 hover:shadow-xl hover:shadow-blue-950/30'
                        : 'border-slate-200 bg-slate-50 hover:border-[#2962ff]/40 hover:shadow-xl hover:shadow-blue-100'
                }`}
            >
                <div className="pointer-events-none absolute -right-10 -top-10 h-32 w-32 rounded-full bg-[#2962ff]/10 opacity-0 blur-2xl transition-opacity duration-300 group-hover:opacity-100" />
                <div className="relative flex items-start justify-between gap-3">
                    <div className="flex min-w-0 items-center gap-3">
                        {coin.fundamentals?.logo_url ? (
                            ...
                            <span className="flex h-10 w-10 items-center justify-center rounded-full bg-[#2962ff]/15 text-xs font-bold text-[#5b8cff]">{base}</span>
                        )}
                    ...
                    {coin.fundamentals?.market_cap_rank && (
                        <span className="rounded bg-[#2962ff]/10 px-2 py-1 text-[10px] font-bold text-[#5b8cff]">Rank #{coin.fundamentals.market_cap_rank}</span>
                    )}
                </div>
    ...
    return (
        <div className={`min-h-screen ${isDark ? 'bg-black-screen-color text-white' : 'bg-slate-50 text-slate-950'}`}>
            <div className="fixed inset-x-0 top-0 z-[60] h-[2px] bg-transparent">
                <div
                    className="h-full bg-gradient-to-r from-[#2962ff] to-[#5b8cff] transition-[width] duration-150"
            ...
            >
                <div className="mx-auto flex max-w-7xl items-center gap-3">
                    ...
                    <div className="hidden flex-[1.7] items-center justify-center gap-2 lg:flex">
                        <div className={`flex h-10 w-[min(320px,30vw)] items-center gap-2 rounded-md border px-3 transition-colors focus-within:border-[#2962ff]/60 ${isDark ? 'border-gray-700 bg-black-table-color text-gray-400' : 'border-slate-200 bg-slate-100 text-slate-500'}`}>
                        ...
                        {navItems.map(([item, href]) => (
                            ...
                            >
                                <span className="relative flex h-full items-center">
                                    {item}
                                    <span className="absolute -bottom-1 left-0 h-[2px] w-0 rounded-full bg-[#2962ff] transition-all duration-300 group-hover:w-full" />
                                </span>
                            </a>
                        ))}
                    </div>
            ...
            <main>
                ...
                >
                    <div
                        ...
                        style={{
                            background: `radial-gradient(600px circle at ${glow.x}% ${glow.y}%, rgba(41,98,255,${isDark ? 0.16 : 0.1}), transparent 45%)`,
                        }}
                    />
                    ...
                    <div className="max-w-2xl">
                        <div className={`mb-5 inline-flex animate-floatY items-center gap-2 rounded-md border px-3 py-2 text-xs font-semibold uppercase tracking-wide ${isDark ? 'border-gray-700 bg-black-table-color text-gray-200' : 'border-gray-300 bg-white text-slate-700'}`}>
                            <Sparkles size={15} className="text-[#5b8cff]" />
                        ...
                        <div className="mt-8 flex flex-wrap gap-3">
                            <Link
                                href="/login"
                                className={`group inline-flex h-11 items-center gap-2 rounded-md px-4 text-sm font-bold transition-all duration-200 hover:-translate-y-0.5 hover:shadow-lg hover:shadow-blue-950/30 ${isDark ? 'bg-white text-skin-black hover:bg-gray-200' : 'bg-skin-black text-white hover:bg-skin-black-light'}`}
                            >
                                Sign in
                    ...
                    <div className={`animate-floatY overflow-hidden rounded-xl border shadow-2xl shadow-blue-950/20 [animation-duration:9s] ${isDark ? 'border-[#2a2e39] bg-[#131722]' : 'border-slate-200 bg-white'}`}>
                        ...
                        <div className="relative h-[330px] overflow-hidden bg-[linear-gradient(rgba(120,123,134,.08)_1px,transparent_1px),linear-gradient(90deg,rgba(120,123,134,.08)_1px,transparent_1px)] bg-[size:48px_48px] sm:h-[420px]">
                            ...
                            <svg viewBox="0 0 700 380" className="absolute inset-0 h-full w-full" preserveAspectRatio="none" aria-label="Trading replay preview">
                                ...
                                />
                                <circle cx="700" cy="72" r="5" fill="#2962ff" className="transition-opacity duration-500" style={{ opacity: chartDrawn ? 1 : 0 }}>
                            ...
                            <div className={`absolute bottom-4 left-4 right-4 grid grid-cols-3 gap-2 rounded-lg border p-2 backdrop-blur ${isDark ? 'border-[#2a2e39] bg-[#0b0e14]/90' : 'border-slate-200 bg-white/90'}`}>
                                {[[Activity, 'Replay', 'Step through price'], [ShieldCheck, 'Risk', 'Plan before entry'], [BookOpen, 'Journal', 'Review the process']].map(([Icon, title, copy]) => (
                                    <div key={title} className="group flex items-center gap-2 rounded-md p-2 transition-colors hover:bg-[#2962ff]/10"><Icon size={16} className="shrink-0 text-[#2962ff] transition-transform duration-200 group-hover:scale-110" /><div><div className="text-[11px] font-bold">{title}</div><div className="hidden text-[9px] text-slate-500 sm:block">{copy}</div></div></div>
                                ))}
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="mx-auto grid max-w-7xl grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
                        {featureItems.map(({ icon: Icon, title, description }, index) => (
                            <Reveal key={title} delay={index * 80}>
                                <div className={`group h-full rounded-md border p-5 transition-all duration-300 hover:-translate-y-1 hover:shadow-lg ${isDark ? 'border-gray-700 bg-black-table-color hover:border-[#2962ff]/50 hover:shadow-blue-950/20' : 'border-slate-200 bg-slate-50 hover:border-[#2962ff]/40 hover:shadow-blue-100'}`}>
                                    <div className={`mb-3 flex h-10 w-10 items-center justify-center rounded-md bg-[#2962ff]/10 text-[#5b8cff] transition-transform duration-300 group-hover:scale-110`}>
                                        <Icon size={20} />
                                    </div>
                ...
                <section id="process" className={`border-t px-4 py-14 ${isDark ? 'border-gray-800 bg-black-screen-color' : 'border-slate-200 bg-slate-50'}`}>
                    <div className="mx-auto max-w-7xl">
                        <Reveal className="mb-8 max-w-xl"><div className="text-xs font-bold uppercase tracking-[0.2em] text-[#2962ff]">One repeatable loop</div><h2 className="mt-2 text-3xl font-bold">A workspace designed around improvement.</h2></Reveal>
                        <div className="relative grid gap-3 md:grid-cols-4">
                            <div className={`pointer-events-none absolute left-0 right-0 top-9 hidden h-px md:block ${isDark ? 'bg-gradient-to-r from-transparent via-[#2962ff]/40 to-transparent' : 'bg-gradient-to-r from-transparent via-[#2962ff]/25 to-transparent'}`} />
                            {processSteps.map((item, index) => (
                                <Reveal key={item} delay={index * 100}>
                                    <div className={`group relative rounded-lg border p-5 transition-all duration-300 hover:-translate-y-1 hover:border-[#2962ff]/50 hover:shadow-lg ${isDark ? 'border-[#2a2e39] bg-[#131722] hover:shadow-blue-950/20' : 'border-slate-200 bg-white hover:shadow-blue-100'}`}>
                                        <div className="mb-5 flex h-8 w-8 items-center justify-center rounded-full bg-[#2962ff]/15 text-xs font-bold text-[#5b8cff] transition-transform duration-300 group-hover:scale-110">0{index + 1}</div>
            ...
            <footer className={`border-t px-4 py-6 ${isDark ? 'border-gray-800 bg-skin-black text-slate-500' : 'border-slate-200 bg-white text-slate-500'}`}>
                <div className="mx-auto flex max-w-7xl flex-col items-center justify-between gap-3 text-xs sm:flex-row">
                    ...
                    <div className="flex gap-5">
                        <Link href="/privacy-policy" className="transition-colors hover:text-[#5b8cff]">Privacy Policy</Link>
                        <Link href="/terms-of-service" className="transition-colors hover:text-[#5b8cff]">Terms of Service</Link>
                    </div>
                </div>
            </footer>
        </div>

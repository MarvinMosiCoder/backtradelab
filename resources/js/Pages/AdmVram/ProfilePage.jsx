                        ...
                        <div className="flex flex-col justify-center items-center">
                            ...
                            <div className={`mt-7 w-full max-w-xl rounded-xl border p-5 text-left ${theme === 'bg-skin-black' ? 'border-[#2a2e39] bg-[#0b0e14] text-[#d1d4dc]' : 'border-slate-200 bg-slate-50 text-slate-900'}`}>
                                ...
                                <div className="grid gap-3 sm:grid-cols-2">
                                    <label className="text-xs font-semibold">Display name<input value={details.name} disabled={nameLocked} onChange={(e)=>setDetails((c)=>({...c,name:e.target.value}))} className="mt-1.5 h-10 w-full rounded-lg border border-[#2a2e39] bg-transparent px-3 text-sm outline-none focus:border-[#2962ff] disabled:opacity-50" required/>{nameLocked ? <span className="mt-1 block text-[10px] font-normal normal-case text-amber-500">Your display name has already been changed and can't be changed again here.</span> : <span className="mt-1 block text-[10px] font-normal normal-case text-[#787b86]">You can change this once — choose carefully, it can't be edited again afterward.</span>}</label>
                                    <label className="text-xs font-semibold">Username<input value={details.username} disabled={usernameLocked} onChange={(e)=>setDetails((c)=>({...c,username:e.target.value}))} className="mt-1.5 h-10 w-full rounded-lg border border-[#2a2e39] bg-transparent px-3 text-sm outline-none focus:border-[#2962ff] disabled:opacity-50" placeholder="trader.name"/>{usernameLocked && <span className="mt-1 block text-[10px] font-normal normal-case text-amber-500">You can change your username again on {usernameNextChangeAt.toLocaleDateString()}.</span>}</label>

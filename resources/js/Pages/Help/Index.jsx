...
export default function HelpIndex() {
  return <><Head title="How to use BacktradeLab"/><div className="mx-auto max-w-4xl space-y-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><h1 className="text-xl font-bold">How to use BacktradeLab</h1><p className="mt-1 text-sm text-[#787b86]">A practical chart-to-journal workflow.</p></div><Link href="/workspace?tour=1" className="rounded-lg bg-[#2962ff] px-4 py-2 text-xs font-bold text-white">Restart workspace tour</Link></div><div className="grid gap-3 sm:grid-cols-2">{sections.map(([title, copy]) => <section key={title} className="rounded-lg border border-[#2a2e39] bg-[#131722] p-4"><h2 className="font-semibold text-white">{title}</h2><p className="mt-2 text-sm leading-6 text-[#b2b5be]">{copy}</p></section>)}</div></div></>;
}
